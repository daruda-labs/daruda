...
**Tier 3 — transient / live state: NOT reachable by config or state alone.** Restore recomputes these fresh. The `--screenshot-scenario <name>` flag drives one transient overlay into view after settle and before capture (it forces a repaint since `render_to_image` captures the last *painted* frame). Implemented scenarios: `command-palette`, `error-modal`, `settings` / `settings:<section-slug>` (Settings is a separate window — the scenario captures *that* window; slug = `BuiltinSection::slug`, e.g. `font`, `keymap`, `notifications`), `toast`, `pane-context-menu` (the focused pane's right-click menu — the only way to eyeball menu length, edge-flip and the keybinding column). Add a variant to `ScreenshotScenario` + `screenshot_scenario::drive` (`crates/app/src/workspace/screenshot_scenario.rs`) to cover more. The rest below still need a real backing source, not just a scenario:
...
`ferrum_flow` is a vendored copy of `tu6ge/ferrum-flow` at `43b762ce` (Apache-2.0) — the node-graph canvas behind the flow editor. Vendored for the same reason and on the same terms: source copied verbatim, `Cargo.toml` the only daruda-authored file, excluded from the same lint passes, reached only through `crate::ui::*`. Upstream takes `gpui` from crates.io, which resolves to a different crate instance than daruda's pinned zed git rev, so routing it through `gpui = { workspace = true }` is the whole point. It differs from `gpui_component` in one way: it declares `rust-version` and leaves `clippy::incompatible_msrv` armed, so a std API newer than CI's pinned toolchain is caught locally. Provenance and the re-vendor procedure are in `patches/README.md`.
