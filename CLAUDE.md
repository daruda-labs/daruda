...
**Tier 3 — transient / live state: NOT reachable by config or state alone.** Restore recomputes these fresh. The `--screenshot-scenario <name>` flag drives one transient overlay into view after settle and before capture (it forces a repaint since `render_to_image` captures the last *painted* frame). Implemented scenarios: `command-palette`, `error-modal`, `settings` / `settings:<section-slug>` (Settings is a separate window — the scenario captures *that* window; slug = `BuiltinSection::slug`, e.g. `font`, `keymap`, `notifications`), `toast`, `pane-context-menu` (the focused pane's right-click menu — the only way to eyeball menu length, edge-flip and the keybinding column), `mermaid-lightbox`, the agent-chat set — `agent-chat` (a settled transcript in its default state), `agent-chat-narrowed` (the same one with the display filter and tail window engaged, so both placeholder rows render together) and `agent-chat-failure` (a pane parked on an expired login) — and the flow set — `flow-picker`, `flow-profile-picker`, `flow-running`, `flow-asking`, `flow-resumable`, `flow-graph`, `flow-graph-running`, `flow-graph-form`, `flow-graph-form-refused`, `flow-graph-pinned`, `flow-graph-authoring`, `flow-delete-confirm`. `ScreenshotScenario::from_cli_name` is the list that decides; this one is prose about it. The agent-chat set is seeded, not connected: `shot_transcript::sample_transcript()` is a fixed conversation (mixed tool kinds, a failed call, one diff, more work cycles than the largest tail window keeps) that reaches the pane through the single `AgentChatView::seed_transcript_for_shot` entry point, so **no ACP session is needed** to shoot the Step layer, the activity-bar chips or the status rollup. Narrowing is driven through the ops the chips call, not by writing the fields, so a capture shows what a user's clicks produce. Add a variant to `ScreenshotScenario` + `screenshot_scenario::drive` (`crates/app/src/workspace/screenshot_scenario.rs`) to cover more. The rest below still need a real backing source, not just a scenario:
