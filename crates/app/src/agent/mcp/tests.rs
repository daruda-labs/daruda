...
use super::{
    McpScope, McpServer, McpTransport, NameError, format_env_lines, parse, parse_env_lines,
    validate_command, validate_name, validate_url,
};
...
fn lossless_round_trip_preserves_unknown_keys() {
    ...
    let (servers, raw) = parse::parse_personal_settings(&path).unwrap();
    ...
    let mut raw = raw;
    set_disabled(&mut raw, &path, McpScope::Personal, "alpha", true).unwrap();
...
fn write_server_appends_without_disturbing_existing_entries() {
    ...
    write_server(&mut raw, &path, McpScope::Project, &draft).unwrap();
...
fn update_server_rewrites_target_only() {
    ...
    update_server(&mut raw, &path, McpScope::Personal, &draft).unwrap();

    let (servers, _) = parse::parse_personal_settings(&path).unwrap();
    let alpha = servers.iter().find(|s| s.name == "alpha").unwrap();
...
fn delete_then_round_trip() {
    ...
    delete_server(&mut raw, &path, McpScope::Personal, "alpha").unwrap();

    let (servers, on_disk) = parse::parse_personal_settings(&path).unwrap();
    assert_eq!(servers.len(), 1);
