...
/// Spawn a watcher that yields one debounced signal per settling point on
/// `panels.json`. Returns the debounced receiver plus the
/// [`crate::dir_watch::DirWatcher`] handle that keeps the watch alive (this is
/// an app-global watcher, so the caller parks the handle for the process
/// lifetime). On an FSEvents drop (sleep/wake) one reload signal is emitted so
/// the panels state is re-read.
pub fn spawn_panels_watcher() -> (mpsc::Receiver<()>, crate::dir_watch::DirWatcher) {
    use notify::RecursiveMode;

    let panels_path =
        daruda_store::panels::panels_path_in(&daruda_store::persistence::default_data_dir());
    // Watch the parent directory — atomic-rename saves (which is what daruda
    // itself does via `tempfile + persist`) replace the inode, so a direct
    // file watch loses its target.
    let watch_dir = panels_path.parent_or_current().to_path_buf();
    let file_name = panels_path.file_name().map(|n| n.to_os_string());

    // Ensure the directory exists so notify can attach to it (otherwise the
    // very first watch attempt fails on a fresh install before daruda has
    // written anything).
