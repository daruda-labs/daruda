...
/// Built once: every visible row asks on every render.
static DEFAULTS: std::sync::LazyLock<Config> = std::sync::LazyLock::new(Config::default);
...
impl SettingsView {
    ...
    pub(super) fn differs_from_default(&self, target: Target) -> bool {
        default_patch(target)
            .is_some_and(|patch| patch.field_changed_between(&self.base_config, &DEFAULTS))
    }
...
mod tests {
    ...
    fn a_default_patch_names_the_field_its_row_edits() {
        ...
        let ports = Target::StatusBarItem(daruda_config::StatusBarItem::Ports);
        assert!(default_patch(ports).is_none());
    }
}
