...
pub struct SettingsWindow {
    ...
    /// The paired chat, mirrored from `SettingsStore`. Pairing is written by
    /// the bridge's poll loop rather than by any editor here, so it is not one
    /// of [`Self::settings_ui_patches`] and the window would otherwise never
    /// learn that a phone paired while it was open. Single update site:
    /// [`Self::adopt_external_settings`].
...
impl SettingsWindow {
    ...
    fn apply_settings_patch_force_as(
        &mut self,
        patch: daruda_config::SettingsPatch,
    ...
    /// deliberately absent: pairing owns it, so there is no editor here to
    /// reload into. The window still has to *see* it change, which is what
    /// [`Self::adopt_external_settings`] mirrors alongside this diff.
    ...
    ///
    /// Two halves, because they read different sources. The pairing is
    /// mirrored straight from the live store — it is not one of
    /// [`Self::settings_ui_patches`], so the field-diff below is blind to it
    /// and a phone that paired while Settings was open would never show up.
    /// Everything else goes through that diff, which also decides whether a
    /// local draft should be left alone.
    fn adopt_external_settings(&mut self, window: &mut Window, cx: &mut Context<Self>) {
