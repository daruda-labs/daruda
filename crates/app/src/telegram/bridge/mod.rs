//! The Telegram bridge's vocabulary: what an inbound update is decided to
//! be, and what the bridge sends back out.
//!
//! Plain data with no policy of its own — [`core`] holds the state machine
//! that produces these and consumes them. Split because the two have
//! different reasons to change: the shapes here move when the surface the
//! phone talks to changes, the routing rules move when the policy does.
//!
//! All I/O happens one layer up in `global`'s poll loop, which owns the
//! [`BridgeCore`] instance. Nothing in this module tree may import `gpui`,
//! `daruda_acp`, or `ureq`.
