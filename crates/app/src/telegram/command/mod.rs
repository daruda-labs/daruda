...
/// two namespaces cannot collide structurally. `pub(super)` because the router
/// needs it to tell "this listing is stale" from "this token is unknown".
pub(super) const LISTING_TOKEN_PREFIX: &str = "lst";
...
#[derive(Clone, Debug)]
pub(super) struct ListingRow {
    pub(super) name: String,
    pub(super) summary: ChatSummary,
}
...
impl CommandState {
    ...
    pub(super) fn rows(&self) -> &[ListingRow] {
        &self.rows
    }
