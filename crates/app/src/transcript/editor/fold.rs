...
pub(crate) type FoldRuleEdit = Rc<dyn Fn(FoldMode, &mut App)>;
pub(crate) type FoldPresetPress = Rc<dyn Fn(Option<FoldPreset>, &mut App)>;
...
fn block_rule_row(
    ...
    rule_row(
        ...
        cx,
        move |rule, app| on_change(mode.with_rule(turn, block, rule), app),
    )
}
...
fn tool_rule_row(
    ...
    rule_row(
        ...
        cx,
        move |rule, app| on_change(mode.with_tool_rule(turn, category, rule), app),
    )
}
