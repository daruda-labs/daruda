...
| Loose lists are actually spaced | `src/text/node.rs` (`Node::List.spread` field + `NodeRenderOptions.loose` + the `render_list_item` gap), `src/text/format/{markdown,html}.rs` (carry `mdast::List.spread` through) | a list whose items are separated by blank lines is *loose* and its items are spaced like paragraphs. Upstream pushed an empty `div()` as the item's first child, which in a gap-less flex column adds no height at all — a loose list rendered pixel-identical to a tight one. Now a non-first item whose list (or whose own children) is spread takes `pt(paragraph_gap)`. The list-level flag had to be added because upstream's parser dropped `mdast::List.spread` and kept only the per-item one, which is false for the common `- a\n\n- b` shape. Regression-tested at `crate::ui::markdown::tests::a_loose_list_spaces_its_items`. |
