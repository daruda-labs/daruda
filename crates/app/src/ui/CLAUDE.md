...
├── disclosure.rs   # disclosure(id, is_open) — stateless chevron toggle; caller owns fold state. `.axis(DisclosureAxis)` picks the glyph pair: `Horizontal` (default, ChevronRight closed / ChevronDown open) for a tree node, `Vertical` (ChevronDown closed / ChevronUp open) for a control that is a *boundary* rather than a node, so it cannot be mistaken for the rows it sits among (the agent chat's tail-window boundary). Plus .color()/.size()/.on_toggle()
...
├── icons.rs        # Material Symbols paths + icon(path): explicit 16px SVG artwork. button_icon / button_icon_danger / button_icon_on_surface add a 24px target; button_with_icon keeps 16px artwork before a text label, independent of the vendor button tier.
...
| PopupMenuItem hover tooltip | `src/menu/popup_menu.rs` (`PopupMenuItem::Item.tooltip` field + `.tooltip()`), `src/menu/menu_item.rs` (`MenuItemElement.tooltip` field + render wiring) | The vendored `PopupMenu` had no hover-tooltip affordance — daruda uses it to explain *why* a disabled menu item is disabled (e.g. "Commit first" on a disabled merge action); daruda's own now-removed `ContextMenuItem` already had it. Same `.tooltip(text)` builder shape as `Switch`/`Button`; renders independent of the disabled-gated click handler, so it survives disabled state. |
...
| TextView tints against the surface it is painted on | `src/text/node.rs` (`tint_over` + `NodeRenderOptions::{surface, tint}`; the four tint sites and the three alphas named as consts), `src/text/text_view.rs` (`TextView::surface_color` builder threaded to `TextViewElement`) | the view's fills and structural lines — inline-code chips, the code-block fill and border, table frame / row / cell separators, the `<hr>` rule — are a neutral overlay whose direction (white over dark, black over light) was read from `cx.theme().background`, the **UI canvas**. On the agent-chat pane that is the wrong surface (DESIGN.md §AgentChatPane), and with a light `ui_preset` over a dark terminal it picked the black tint for a near-black pane: every one of them vanished at once — captured before/after, the table rendered as floating text with no grid. The host now passes the surface it actually paints on; unset keeps the upstream fallback. `crate::ui::markdown` takes it via `.surface(..)`, and the agent-chat call sites go through one `blocks::pane_markdown` helper so colour + surface + size cannot drift apart. Tested at `gpui_component::text::node::tests::a_tint_steps_away_from_the_surface_it_is_drawn_on` (the rule) and `crate::ui::theme::tests::markdown_structural_lines_step_off_the_pane_not_the_ui_canvas` (the combination). |
| TextView asks the host for its muted colour | `src/text/text_view.rs` (`TextView::muted_text_color` builder + `muted_color` threaded to `TextViewElement`), `src/text/node.rs` (`NodeRenderOptions::muted_color`; the blockquote and its bar read it, falling back to `Theme::muted_foreground`) | a blockquote is the one run of prose the view colours itself, and it took `cx.theme().muted_foreground` — a UI-theme tone. DESIGN.md §AgentChatPane rules that out on this pane: `ui_preset` and `terminal_preset` are independent config keys, so a UI colour has no verified contrast on a terminal-mirrored background. Measured on the built-in terminal presets it fell below the 4.5:1 body floor on three of eight (nord 3.85, one_dark 4.32, dracula 4.39), and a light `ui_preset` over any dark terminal put it at 2.1–2.9. `crate::ui::markdown` now derives the colour from the body colour the caller already passes (`color.opacity(MD_VIEW_MUTED_ALPHA)`), so a quote lands on `pane-fg-muted` — the tier every other secondary label on the pane uses — and inherits the caller's per-pane dimming for free. Carried on the element, **not** in `TextViewStyle`: a style change re-parses the document, so a colour that tracks dimming would reparse the whole transcript on every focus change. Regression-tested at `crate::ui::theme::tests::a_markdown_quote_reads_on_the_pane_it_is_painted_on`. |
| TextView one line height for the whole view | `src/text/style.rs` (`TextViewStyle::line_height` field + builder, `paragraph_gap` `Rems` → `AbsoluteLength`), `src/text/text_view.rs` (`TextViewElement::render` applies it to the root), `src/text/node.rs` (`Node::Paragraph` loses its own `.line_height(rems(1.3))`) | every line in the view now inherits one `line_height`, a fraction by default (1.6) so the vertical rhythm follows the host's text size. Upstream mixed two systems: paragraphs carried a hardcoded `rems(1.3)` — an absolute 20.8px that ignored the font size — while bullets, code blocks and table cells were left on gpui's `phi()` (1.618 **em**). Two consequences. (1) A list item's bullet is a bare string child of its `h_flex`, so its taller `phi()` line box set the row height of every *single-line* item while a wrapped item was driven by the paragraph: at a 15px body, one-line items sat at 24.5px and the lines inside a two-line item at 20.8px — the same list rendered at two pitches, and a `- [ ]` task row (rem-sized checkbox, no bullet) disagreed with the `-` rows beside it. (2) Raising `font.agent_chat.size` grew the glyphs but not the leading (1.16× at 18px). Regression-tested at `crate::ui::markdown::tests::{a_single_line_list_item_is_one_line_tall, a_bullet_item_and_a_task_item_share_a_row_height, a_wrapped_list_keeps_the_pitch_of_an_unwrapped_one, a_code_block_shares_the_prose_line_pitch, the_vertical_metrics_follow_the_text_size}`. |
| Loose lists are actually spaced | `src/text/node.rs` (`Node::List.spread` field + `NodeRenderOptions.loose` + the `render_list_item` gap), `src/text/format/{markdown,html}.rs` (carry `mdast::List.spread` through) | a list whose items are separated by blank lines is *loose* and its items are spaced like paragraphs. Upstream pushed an empty `div()` as the item's first child, which in a gap-less flex column adds no height at all — a loose list rendered pixel-identical to a tight one. Now the `Node::List` render arm derives looseness once — `list.spread || any item.spread` — and every non-first item takes `pt(paragraph_gap)`. Both halves are needed because CommonMark calls a list loose two ways and mdast splits them: blank lines *between* items set the list's `spread` (which upstream's parser dropped entirely, so the common `- a\n\n- b` shape was flat), while an item holding two blocks with a blank line between them sets only that *item*'s (so `- a\n\n  cont\n- b` was flat too, even after the list flag was added). Regression-tested at `crate::ui::markdown::tests::{a_loose_list_spaces_its_items, an_item_holding_two_blocks_makes_its_list_loose}` — one per half. |
| TextView Markdown conformance probes | `src/text/{inline,text_view,node}.rs` (`TextView::debug_inline_bounds`, `markdown-task-checkbox`) | The app's shared two-renderer conformance probe observes the exact bounds passed to each prose `Inline::paint` through an unset-by-default callback; measuring the outer paragraph div would stay positive even if its text child collapsed. The task checkbox uses GPUI's `debug_selector`, which is a no-op without `test-support`. Neither probe adds a layout wrapper. |
...
| Expose the right-clicked link | `src/global_state.rs` (`GlobalState::right_clicked_link: Option<SharedString>`), `src/text/inline.rs` (a **capture**-phase `MouseDownEvent` handler writes it), `src/text/text_view.rs` (`pub fn take_right_clicked_link(cx, at) -> Option<SharedString>` and its mirror `pub fn record_right_clicked_link(cx, at, url)`, for a host element that is not inline text — the tool card's resource-link button records its own URL on a right press, so the pane menu treats it like a Markdown link) | A link in agent chat is rendered by a hand-rolled `Element`, so the host has no way to ask what sits under a right press — and the host's own right-press handler is what opens the pane context menu. Same shape as zed's `Markdown::capture_for_context_menu` / `context_menu_link()`. Capture phase is the whole trick: gpui runs every capture listener before any bubble one (`window.rs`, `dispatch_mouse_event`), so the URL is recorded before the host's bubble handler builds its menu, whichever order the two were registered in. A **global** rather than view state, unlike zed: zed's `Markdown` is an `Entity` the host already holds, while an `Inline` is an element built fresh each frame with no handle the host can reach — the global is the only channel there is. Two things keep that from rotting: the accessor *takes* (`Workspace::take_pane_click_info` consumes it on every pane menu, used or not), and the record carries the **press position**, which the caller must match — so a record no menu consumed is refused rather than adopted by an unrelated pane's menu. Returns `None` before `gpui_component::init`, so a host that opens a menu without the vendored globals installed does not panic. Drives the chat's Open in File View / Open Externally / Copy Link Address entries. |
