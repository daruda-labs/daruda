...
| Shared word-boundary module | `src/text_selection.rs` (new module, `pub mod text_selection` in `src/lib.rs`) | `pub enum CharType { Word, Whitespace, Newline, Other }` + `pub fn word_range(len, char_at, offset) -> Option<Range<usize>>` — generic byte-offset word-boundary logic shared by the input widget and future plain-text consumers (`&str`). `char_at` is a closure `Fn(usize) -> Option<char>` keyed by byte offset, so both `RopeExt::char_at` and `str.is_char_boundary`/`chars().next()` satisfy it without any ropey import in the shared module. |
