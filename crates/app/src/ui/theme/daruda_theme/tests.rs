...
fn default_clones_compile_time_palette() {
    ...
    assert_eq!(t.tab_inactive_bg, palette::TAB_INACTIVE_BG);
    assert_eq!(t.tab_inactive_hover_bg, palette::TAB_INACTIVE_HOVER_BG);
    assert_eq!(t.tab_inactive_text, palette::TAB_INACTIVE_TEXT);
    assert_eq!(t.status_bar_bg, palette::STATUS_BAR_BG);
    ...
    assert_eq!(t.dock_placeholder_text, palette::DOCK_PLACEHOLDER_TEXT);
    assert_eq!(t.button_widget_bg, palette::BUTTON_WIDGET_BG);
    assert_eq!(t.button_widget_bg_hover, palette::BUTTON_WIDGET_BG_HOVER);
    assert_eq!(t.button_widget_text, palette::BUTTON_WIDGET_TEXT);
    assert_eq!(
        t.panel_tab_drop_target_bg,
        palette::PANEL_TAB_DROP_TARGET_BG
    );
    assert_eq!(t.muted_text, palette::MUTED_TEXT);
    assert_eq!(t.faint_text, palette::FAINT_TEXT);
    assert_eq!(t.disabled_item_bg, palette::DISABLED_ITEM_BG);
    assert_eq!(t.disabled_item_text, palette::DISABLED_ITEM_TEXT);
    assert_eq!(t.close_button_hover_bg, palette::CLOSE_BUTTON_HOVER_BG);
    assert_eq!(t.accent_green, palette::ACCENT_GREEN);
    assert_eq!(t.dock_icon_inactive, palette::DOCK_ICON_INACTIVE);
    assert_eq!(t.dock_icon_hover, palette::DOCK_ICON_HOVER);
    assert_eq!(t.dock_icon_active_bg, palette::DOCK_ICON_ACTIVE_BG);
    assert_eq!(t.dock_view_tab_inactive, palette::DOCK_VIEW_TAB_INACTIVE);
    ...
    assert_eq!(t.modal_input_border, palette::MODAL_INPUT_BORDER);
    ...
    assert_eq!(t.modal_secondary_text, palette::MODAL_SECONDARY_TEXT);
    ...
    assert_eq!(
        ...
        palette::SETTINGS_SCROLLBAR_THUMB
    );
    assert_eq!(
        t.settings_scrollbar_thumb_hover,
        palette::SETTINGS_SCROLLBAR_THUMB_HOVER
    );
    assert_eq!(
        t.right_panel_scrollbar_thumb,
        palette::RIGHT_PANEL_SCROLLBAR_THUMB
    );
    assert_eq!(
        t.right_panel_scrollbar_thumb_hover,
        palette::RIGHT_PANEL_SCROLLBAR_THUMB_HOVER
    );
    ...
    assert_eq!(t.palette_bg, palette::PALETTE_BG);
    assert_eq!(t.palette_border, palette::PALETTE_BORDER);
    assert_eq!(t.palette_input_border, palette::PALETTE_INPUT_BORDER);
    assert_eq!(t.palette_focused_bg, palette::PALETTE_FOCUSED_BG);
    ...
    assert_eq!(
        t.git_file_row_selected_bg,
        palette::GIT_FILE_ROW_SELECTED_BG
    );
    assert_eq!(t.git_file_row_hover_bg, palette::GIT_FILE_ROW_HOVER_BG);
    assert_eq!(t.git_diff_panel_bg, palette::GIT_DIFF_PANEL_BG);
    assert_eq!(t.git_diff_border, palette::GIT_DIFF_BORDER);
    ...
    assert_eq!(t.git_diff_context_text, palette::GIT_DIFF_CONTEXT_TEXT);
    assert_eq!(t.git_diff_hunk_bg, palette::GIT_DIFF_HUNK_BG);
    assert_eq!(
        t.git_stage_checkbox_border,
        palette::GIT_STAGE_CHECKBOX_BORDER
    );
    assert_eq!(
    ...
    assert_eq!(
        t.git_stage_checkbox_unchecked_bg,
        palette::GIT_STAGE_CHECKBOX_UNCHECKED_BG
    );
    assert_eq!(t.git_commit_border, palette::GIT_COMMIT_BORDER);
    ...
    assert_eq!(
        t.pane_header_unfocused_bg,
        palette::PANE_HEADER_UNFOCUSED_BG
    );
    assert_eq!(t.pane_header_cwd_text, palette::PANE_HEADER_CWD_TEXT);
    ...
    assert_eq!(t.pane_divider_bg, palette::PANE_DIVIDER_BG);
    assert_eq!(t.welcome_bg, palette::WELCOME_BG);
    assert_eq!(t.welcome_button_bg, palette::WELCOME_BUTTON_BG);
    assert_eq!(t.welcome_button_hover_bg, palette::WELCOME_BUTTON_HOVER_BG);
    assert_eq!(t.welcome_button_border, palette::WELCOME_BUTTON_BORDER);
    assert_eq!(t.welcome_recent_hover_bg, palette::WELCOME_RECENT_HOVER_BG);
    assert_eq!(t.welcome_text, palette::WELCOME_TEXT);
    assert_eq!(t.file_viewer_bg, palette::FILE_VIEWER_BG);
    assert_eq!(t.file_viewer_header_bg, palette::FILE_VIEWER_HEADER_BG);
    assert_eq!(
        t.file_viewer_header_border,
        palette::FILE_VIEWER_HEADER_BORDER
    );
    assert_eq!(t.file_viewer_header_text, palette::FILE_VIEWER_HEADER_TEXT);
    assert_eq!(t.file_viewer_close_hover, palette::FILE_VIEWER_CLOSE_HOVER);
    assert_eq!(t.file_viewer_text, palette::FILE_VIEWER_TEXT);
    assert_eq!(
        t.file_viewer_selection_bg,
        palette::FILE_VIEWER_SELECTION_BG
    );
    assert_eq!(
        t.file_viewer_line_no_text,
        palette::FILE_VIEWER_LINE_NO_TEXT
    );
    assert_eq!(
        t.file_viewer_tab_active_bg,
        palette::FILE_VIEWER_TAB_ACTIVE_BG
    );
    assert_eq!(t.file_viewer_tab_text, palette::FILE_VIEWER_TAB_TEXT);
    assert_eq!(
    ...
    assert_eq!(t.file_diff_hunk_bg, palette::FILE_DIFF_HUNK_BG);
    assert_eq!(t.file_diff_hunk_text, palette::FILE_DIFF_HUNK_TEXT);
    assert_eq!(t.file_diff_hunk_border, palette::FILE_DIFF_HUNK_BORDER);
    assert_eq!(t.file_diff_ctx_text, palette::FILE_DIFF_CTX_TEXT);
    assert_eq!(t.file_viewer_divider, palette::FILE_VIEWER_DIVIDER);
    assert_eq!(
        t.file_viewer_scrollbar_thumb,
        palette::FILE_VIEWER_SCROLLBAR_THUMB
    );
    assert_eq!(
        t.file_viewer_scrollbar_thumb_hover,
        palette::FILE_VIEWER_SCROLLBAR_THUMB_HOVER
    );
    ...
    assert_eq!(t.file_viewer_search_text, palette::FILE_VIEWER_SEARCH_TEXT);
    assert_eq!(
        t.file_viewer_search_count,
        palette::FILE_VIEWER_SEARCH_COUNT
    );
    assert_eq!(
    ...
    assert_eq!(t.md_h1_color, palette::MD_H1_COLOR);
    assert_eq!(t.md_h2_color, palette::MD_H2_COLOR);
    assert_eq!(t.md_h3_color, palette::MD_H3_COLOR);
    assert_eq!(t.md_h4_color, palette::MD_H4_COLOR);
    assert_eq!(t.md_code_inline_bg, palette::MD_CODE_INLINE_BG);
    assert_eq!(t.md_code_inline_text, palette::MD_CODE_INLINE_TEXT);
    assert_eq!(t.md_code_block_bg, palette::MD_CODE_BLOCK_BG);
    assert_eq!(t.md_code_block_border, palette::MD_CODE_BLOCK_BORDER);
    assert_eq!(t.md_blockquote_border, palette::MD_BLOCKQUOTE_BORDER);
    assert_eq!(t.md_blockquote_text, palette::MD_BLOCKQUOTE_TEXT);
    assert_eq!(t.md_rule_color, palette::MD_RULE_COLOR);
    ...
    assert_eq!(t.md_table_border, palette::MD_TABLE_BORDER);
    assert_eq!(t.md_table_header_bg, palette::MD_TABLE_HEADER_BG);
    assert_eq!(t.md_table_row_bg_even, palette::MD_TABLE_ROW_BG_EVEN);
    assert_eq!(t.md_table_row_bg_odd, palette::MD_TABLE_ROW_BG_ODD);
    assert_eq!(t.toast_bg, palette::TOAST_BG);
    assert_eq!(t.toast_border, palette::TOAST_BORDER);
    assert_eq!(t.toast_text, palette::TOAST_TEXT);
    assert_eq!(t.toast_action_text, palette::TOAST_ACTION_TEXT);
    ...
    assert_eq!(t.toast_repeat_bg, palette::TOAST_REPEAT_BG);
    assert_eq!(t.error_modal_body_bg, palette::ERROR_MODAL_BODY_BG);
    assert_eq!(t.error_modal_body_border, palette::ERROR_MODAL_BODY_BORDER);
    assert_eq!(t.terminal_bg, palette::TERMINAL_BG);
    assert_eq!(
        t.terminal_scrollbar_track_bg,
        palette::TERMINAL_SCROLLBAR_TRACK_BG
    );
    assert_eq!(t.keystroke_input_bg, palette::KEYSTROKE_INPUT_BG);
    assert_eq!(t.keystroke_input_border, palette::KEYSTROKE_INPUT_BORDER);
    assert_eq!(
        t.keystroke_input_border_active,
        palette::KEYSTROKE_INPUT_BORDER_ACTIVE
    );
    assert_eq!(t.keystroke_badge_bg, palette::KEYSTROKE_BADGE_BG);
    assert_eq!(t.keystroke_badge_border, palette::KEYSTROKE_BADGE_BORDER);
    assert_eq!(t.keystroke_badge_text, palette::KEYSTROKE_BADGE_TEXT);
    assert_eq!(t.keystroke_hint_text, palette::KEYSTROKE_HINT_TEXT);
    assert_eq!(t.popover_bg, palette::POPOVER_BG);
    assert_eq!(t.popover_border, palette::POPOVER_BORDER);
    ...
    assert_eq!(
        ...
        palette::RIGHT_PANEL_TASK_RUNNING_COLOR
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        palette::TASK_EDIT_BRANCH_INVALID_BORDER
    );
    assert_eq!(t.badge_bg, palette::BADGE_BG);
    assert_eq!(t.badge_border, palette::BADGE_BORDER);
    assert_eq!(t.badge_text, palette::BADGE_TEXT);
    assert_eq!(t.divider_default_color, palette::DIVIDER_DEFAULT_COLOR);
    ...
    assert_eq!(t.gauge_track_bg, palette::GAUGE_TRACK_BG);
    ...
    assert_eq!(t.skill_row_hover_bg, palette::SKILL_ROW_HOVER_BG);
