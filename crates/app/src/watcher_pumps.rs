//! Background-pump helpers for detached watcher loops registered by
//! `watchers_lifecycle`. Each helper covers one of three loop shapes:
//!
//! 1. [`spawn_drain_burst_pump`] — burst-debounced `mpsc::Receiver<()>`
//!    where many notifications collapse into one global apply. Used by
//!    `panels_watcher` via `watchers_lifecycle::spawn_panels_reload`.
