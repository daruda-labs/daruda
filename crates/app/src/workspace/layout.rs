...
pub(super) const DIVIDER_PX: f32 = 1.0;
...
impl PaneLayout {
    pub(super) fn new_split(direction: SplitDirection, children: Vec<PaneLayout>) -> PaneLayout {
        let n = children.len();
    ...
    pub(super) fn next_pane(&self, current: PaneId) -> Option<PaneId> {
        let ids = self.pane_ids();
        let pos = ids.iter().position(|&id| id == current)?;
        Some(ids[(pos + 1) % ids.len()])
    }

    pub(super) fn prev_pane(&self, current: PaneId) -> Option<PaneId> {
        let ids = self.pane_ids();
        let pos = ids.iter().position(|&id| id == current)?;
...
pub(super) fn remove_pane_from_layout(layout: &mut PaneLayout, target: PaneId) -> bool {
...
pub(super) fn cleanup_after_remove(layout: &mut PaneLayout) {
...
pub(super) fn parent_axis_extent(
    layout: &PaneLayout,
    width: f32,
    height: f32,
