...
use crate::workspace::left_sidebar::git_status_ops::{git_status_color, git_status_symbol};
...
fn build_files_scrollbar(
    ...
    let thumb_h = raw_thumb_h.max(px(theme::SIDEBAR_SCROLLBAR_MIN_THUMB_H));
    ...
    let thumb_w = px(theme::SIDEBAR_SCROLLBAR_W);
    ...
    Some(
        div()
            ...
            .absolute()
            .top(thumb_top)
            .right(px(theme::SIDEBAR_SCROLLBAR_MARGIN_R))
            .w(thumb_w)
            .h(thumb_h)
            .rounded(thumb_w / 2.0)
            .bg(thumb_bg)
