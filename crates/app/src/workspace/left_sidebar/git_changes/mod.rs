//! Git Changes sidebar view — unified file list with commit / push controls.
//!
...
use crate::workspace::layout::LeftSidebarSnapshot;
use crate::workspace::left_sidebar::git_status_ops::git_status_color;
...
/// Repo-root-relative paths in the same order the sidebar renders them,
...
fn summary_bar(
    ...
    let toggle_hover = t.sidebar_view_tab_active;
...
fn unified_file_row(
    ...
    let status_symbol = crate::workspace::left_sidebar::git_status_ops::git_status_symbol(status_char);
    ...
    let checkbox_unchecked_bg = t.git_stage_checkbox_unchecked_bg;
    let checkbox_tick_color = t.modal_text_primary;
    let cursor_border_color = t.sidebar_view_tab_accent;
...
fn git_changes_scrollbar(handle: &gpui::ScrollHandle, cx: &gpui::App) -> Option<gpui::AnyElement> {
    ...
    let thumb_h = (viewport_h * thumb_ratio).max(px(theme::SIDEBAR_SCROLLBAR_MIN_THUMB_H));
    ...
    let w = px(theme::SIDEBAR_SCROLLBAR_W);
    let r = px(theme::SIDEBAR_SCROLLBAR_MARGIN_R);
    let t = theme::current(cx);
    let thumb_bg = t.sidebar_scrollbar_thumb;
    let thumb_hover_bg = t.sidebar_scrollbar_thumb_hover;
    Some(
        div()
