...
pub(super) struct FileContent {
    ...
    pub(super) search_input: Entity<crate::ui::InputState>,
...
pub(super) struct TaskEditContent {
    pub(super) task_id: Option<TaskId>,
    pub(super) title_input: Entity<crate::ui::InputState>,
    pub(super) branch_input: Entity<crate::ui::InputState>,
    ...
    pub(super) base_select: Entity<crate::ui::select::SelectState>,
    ...
    pub(super) prompt_state: Entity<gpui_component::input::InputState>,
    pub(super) notes_state: Entity<gpui_component::input::InputState>,
    ...
    /// additions.
    pub(super) new_subtask_input: Entity<crate::ui::InputState>,
...
pub(super) struct TaskEditSnapshot {
...
impl TaskEditContent {
    ...
    /// a user edit.
    pub(super) fn current_snapshot(&self, cx: &App) -> TaskEditSnapshot {
        TaskEditSnapshot {
...
impl Pane {
    ...
    pub(super) fn display_cwd(&self) -> Option<SharedString> {
    ...
    pub(super) fn tab_dirty_dot(&self, cx: &App) -> bool {
        self.is_dirty(cx)
    }

    /// Convenience accessor — file viewer data only.
    pub(super) fn file_view(&self) -> Option<&PaneFileView> {
        self.file_content().map(|f| &f.view)
    }

    /// Mutable convenience accessor for the file viewer data.
    pub(super) fn file_view_mut(&mut self) -> Option<&mut PaneFileView> {
...
fn handle_view_event(
    ...
    let suppressed_by_focus = workspace.notifications.skip_focused_pane
        ...
        && workspace.focused_pane_id == pane_id;
...
pub(super) struct TabEntry {
    ...
    pub(super) user_label: Option<SharedString>,
}
...
pub(super) struct CwdCandidates {
...
/// a reported cwd.
pub(super) fn resolve_default_cwd(inherit_cwd: bool, candidates: CwdCandidates) -> Option<PathBuf> {
...
impl Workspace {
    ...
    /// [`resolve_default_cwd`].
    pub(super) fn default_cwd_for_new_pane(&self) -> Option<PathBuf> {
    ...
    /// inheritance rules.
    pub(super) fn create_pane_with_cwd(
        &mut self,
        cwd: Option<PathBuf>,
        window: &mut Window,
        cx: &mut Context<Self>,
                ...
                if !batch.is_empty() {
                    let ok = cx.update(|_, cx| {
                        ...
                        workspace.update(cx, |ws, cx| {
                            if let Some(pane) = ws.panes.iter_mut().find(|p| p.id == pane_id)
    ...
    pub(super) fn focus_pane(&self, pane_id: PaneId, window: &mut Window, cx: &mut Context<Self>) {
