...
use super::Workspace;
use super::right_panel::task_picker_modal::{TaskPickAction, TaskPickerModal};
...
impl Workspace {
    ...
    pub(in crate::workspace) fn ensure_task_live_tick(&mut self, cx: &mut Context<Self>) {
        ...
        if has_running {
            if self._task_live_tick.is_none() {
                self._task_live_tick = Some(super::task_workflow_ops::spawn_task_live_tick(cx));
            }
        } else {
    ...
    /// removed — the user takes care of that from the sidebar so two
