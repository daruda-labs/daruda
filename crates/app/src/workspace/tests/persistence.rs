...
fn test_restore_state_rebuilds_horizontal_split(cx: &mut TestAppContext) {
    ...
    ws.read_with(cx, |ws, _| {
        ...
        assert!(ids.contains(&ws.active_runtime().focused_pane_id));
    });
}
...
fn test_restore_state_rebuilds_multiple_tabs(cx: &mut TestAppContext) {
    ...
    ws.read_with(cx, |ws, _| {
        ...
        let active_leaves = ws.active_runtime().tabs[2].layout.pane_ids();
        assert!(active_leaves.contains(&ws.active_runtime().focused_pane_id));
    });
}
...
fn test_save_restore_round_trip_preserves_layout(cx: &mut TestAppContext) {
    ...
    ws2.read_with(cx, |ws, _| {
        ...
        assert_eq!(ws.active_runtime().tabs[0].layout.leaf_count(), 2);
        assert_eq!(ws.active_runtime().tabs[1].layout.leaf_count(), 1);
        assert_eq!(ws.active_runtime().panes.len(), 3);
    });
}
