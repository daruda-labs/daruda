use gpui::{
    AnyElement, App, Anchor, Div, Edges, ElementId, InteractiveElement, IntoElement, ParentElement,
...
pub struct TabBar {
    ...
    /// Optional override for the inner `h_flex` gap that controls the
    /// spacing between adjacent tab boxes. When `None`, the variant-
    /// derived default is used (see `RenderOnce::render`).
    inner_gap: Option<Pixels>,
}
...
impl TabBar {
    ...
    /// Override the inner-tabs gap (the spacing between adjacent tab
    /// boxes inside the strip). This shadows the `Styled::gap`
    /// inherent and routes the value to the inner `h_flex` instead of
    /// the outer container. When unset, the variant-derived default
    /// is used.
    pub fn gap(mut self, gap: impl Into<Pixels>) -> Self {
        self.inner_gap = Some(gap.into());
        self
    }
...
impl RenderOnce for TabBar {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let inner_gap_override = self.inner_gap;
        let (bg, paddings, variant_gap) = match self.variant {
            TabVariant::Tab => {
        ...
        self.base
            ...
            .child(
                h_flex()
                    ...
                    .gap(inner_gap_override.unwrap_or(variant_gap))
